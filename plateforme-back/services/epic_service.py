...
class EpicService:
    ...
    def _get_epic_ou_404(self, epic_id: int, projet_id: int):
        epic = self.repo.get_by_id_in_projet(epic_id, projet_id)
        if not epic:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
    ...
    def creer_epic(
        ...
        if not module:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Aucun module trouvé pour le projet {projet_id}.",
            )
    ...
    def get_epics(self, projet_id: int, statut: Optional[str] = None):
        """Liste des epics d'un projet, triés par priorité (optionnellement filtrés)."""
        self._verifier_projet(projet_id)
        if statut:
            ...
            epics = self.repo.get_by_statut(projet_id, statut)
            return self._apply_reference_to_epics_userstories(projet_id, epics)
        epics = self.repo.get_by_projet_with_userstories(projet_id)
        return self._apply_reference_to_epics_userstories(projet_id, epics)

    def get_epic(self, projet_id: int, epic_id: int):
        """Récupérer un epic avec ses user-stories."""
        self._verifier_projet(projet_id)
        epic = self._get_epic_ou_404(epic_id, projet_id)
        return self._apply_reference_to_epic_userstories(projet_id, epic)

    def get_progression(self, projet_id: int, epic_id: int):
    ...
    def modifier_epic(
        ...
        self._verifier_projet(projet_id)
        self._get_epic_ou_404(epic_id, projet_id)
